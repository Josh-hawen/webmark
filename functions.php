...
// 	wp_enqueue_script('shiv', get_template_directory_uri() . '/js/html5shiv.js');
// 	wp_enqueue_script('user_script', get_template_directory_uri() . '/js/main.js');
// 	wp_enqueue_script('user_script_ajax', get_template_directory_uri() . '/js/ajax.js');
// 	// wp_enqueue_style('user_style', get_stylesheet_uri() );
// }
...
function register_my_widgets(){
	register_sidebar( array(
		...
		'description' => 'Этот виджет для переключения языков',
		'before_widget' => '',
		'after_widget' => ''
	) );
}
...
function breadcrumbs() {
  ...
  $sep_before = '<span class="glyphicon glyphicon-menu-right glyphicon-menu-right-breadcrumb">'; // тег перед разделителем
  $sep_after = '</span>'; // тег после разделителя
  $show_home_link = 1; // 1 - показывать ссылку "Главная", 0 - не показывать
  $show_on_home = 0; // 1 - показывать "хлебные крошки" на главной странице, 0 - не показывать
  $show_current = 1; // 1 - показывать название текущей страницы, 0 - не показывать
  $before = '<span class="active">'; // тег перед текущей "крошкой"
